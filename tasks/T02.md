...
ENHANCE the shipped `/start` handler module `src/handlers/start.ts` (a default-exported grammY `Composer`, auto-loaded by the bot core): make `/start` send a welcome message and the bot's main menu (inline keyboard) routing to its top-level features. Do NOT edit the shared `src/bot.ts`. Write the `/start` dialog spec to your OWN `tests/specs/T02.json` (a BotSpec array) and declare the command in your OWN `tests/commands/T02.json` (a JSON string array, e.g. `["/start"]`) — never a shared file.
...
If this task adds a bot command, declare it in its OWN file `tests/commands/T02.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/T02.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
