# fix-b75a723274c09ed5 — Duplicate `/help` command handlers (T01 + T03) fire simultaneously, producing two inconsistent replies

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 QKIT

`src/handlers/T01.ts` and `src/handlers/T03.ts` both register `composer.command("help", …)`. T01 replies: `"…/help — Show this help message"`. T03 replies: `"…/help — Show available commands"`. In grammY's middleware chain, the filter calls `next()` after every handler, so **both** fire for a single `/help` message. The user receives two slightly different help messages. The load order (alphabetic by filename: T01 before T03) determines which fires first, but both always execute.

**Fix:** Remove the redundant `/help` handler (one of T01 or T03) so exactly one composer owns the `/help` command.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-b75a723274c09ed5.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-b75a723274c09ed5.json` (and any new command `tests/commands/fix-b75a723274c09ed5.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-b75a723274c09ed5.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-b75a723274c09ed5.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
