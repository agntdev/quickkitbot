...
Implement the /echo command handler in `src/handlers/echo.ts`:
- Reply with provided text if present.
- If no text, prompt user to reply to the bot's message and detect `reply_to_message` to echo.
Write test spec in `tests/specs/E1T3.json` covering both scenarios and add `/echo` to `tests/commands.json`.
